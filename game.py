...
current_world = "world1"  # Cambia a "world2" para probar el Nivel 2 directamente
current_state = "menu"    # Pon "world" para saltarte el menú en las pruebas
# ---------------------------------
...
#muralla
    ...
    npc_guardia_2.force_fight = False
...
# 1. CARGAMOS AL JEFE SEGÚN EL MUNDO
            if crown is None:  # first frame of fight
                ...
                if current_world == "world1":
                    ...
                    combat.fight_start_time = pygame.time.get_ticks()       # Guarda el momento exacto de inicio
                    combat.next_launch_time = pygame.time.get_ticks() + 1000 # Sincroniza el reloj de los proyectiles
                    ###############
                    print("\n[DEBUG] --- ¡EL COMBATE HA EMPEZADO FÍSICAMENTE! ---")
                    print(f"[DEBUG] Mundo actual: {current_world}")
                    print(f"[DEBUG] Espadas en la cola al arrancar: {len(combat.sword_queue)}")
                    ##########
                    #  AÑADE ESTAS 3 LÍNEAS AQUÍ:
                    combat.init_swords()                                    # Re-pobla las 4 espadas en caliente
                    combat.fight_start_time = pygame.time.get_ticks()       # Guarda el tiempo exacto de inicio
                    combat.next_launch_time = pygame.time.get_ticks() + 500 # Sincroniza el reloj de los proyectiles
...
# 2. MAGIA: GESTIÓN DE ESPADAS SEGÚN EL MUNDO
            if current_world == "world1":
                ...
                #  CAMBIA ESTA CONDICIÓN (Añadimos el seguro de 1000 milisegundos):
                if pygame.time.get_ticks() - combat.fight_start_time > 1000:
                    if len(combat.sword_queue) == 0 and len(combat.swords) == 0:
                        print(f"[DEBUG] ¡VICTORIA DETECTADA! No quedan espadas. Cola: {len(combat.sword_queue)}, En pantalla: {len(combat.swords)}")
                        crown.defeat = True # ¡Fuerzas la victoria automáticamente!
            elif current_world == "world2":
                import random
...
# --- DIBUJAR ELEMENTOS DE LA BATALLA ---
            crown.draw(screen)
            crown.update(proj)
            crown.draw_health_bar(screen)

            # 💥 EL ARREGLO: Todo lo que viene a continuación AHORA SÍ está dentro del IF
            if crown.defeat:
                print("[DEBUG] crown.defeat es TRUE. Entrando al bloque para cargar imagen y pasar al Mundo 2.\n")

                # 1. Cargamos la imagen según el mundo actual
                if current_world == "world1":
                    img_guanyar = pygame.image.load("ride/win1.png").convert()
                elif current_world == "world2":
                    img_guanyar = pygame.image.load("ride/win2.png").convert()
                
                # 2. Mostramos la imagen de victoria en la pantalla durante 3 segundos
                screen.blit(img_guanyar, (0, 3))
                pygame.display.flip()
                pygame.time.delay(3000) 
                
                # 3. Limpiamos el combate de la memoria
                combat.end_fight()
                
                # 4. Cambiamos de mundo o cerramos el juego de forma legal
                if current_world == "world1":
                    combat.change_world = 1
                    crown = None # Destruimos este boss para el siguiente nivel
                else:
                    pygame.quit()
                    os.execl(sys.executable, sys.executable, *sys.argv)

            # TANTE: Esto se queda fuera del IF para que la cuadrícula se dibuje 
            # únicamente MIENTRAS estás luchando perfectamente
            for p in proj:
                p.draw(screen)
